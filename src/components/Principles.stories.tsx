...
export const Functional: Story = () => <ol>
    <li>Validation
        <ol>
            <li>Submitting a form should trigger form validation for all fields</li>
            <li>Validation error messages should appear when invalid fields gets blurred or when invalid character type entered (i.e. text instead of number)</li>
        </ol>
    </li>
    <li>Submitting
        <ol>
            <li>Enter key should submit form</li>
            <li>Submit button text should visibly show the user when the submit is in progress, e.g. append with "...", so "Update preferences" changes to "Updating preferences..."</li>
...
export const Technical: Story = () => <ol>
    <li>Use &lt;form autocomplete=&rdquo;on&rdquo;/&gt; to prepopulate fields previously entered by the user</li>
    <li>Use correct inputmode attributes for form fields to provide appropriate keyboard for users. Input type should usually be text for numbers (others too?) so that desktop browsers don’t ignore incorrect text entry from users (which confuses users), e.g.
    ...
    <li>Use &lt;input name=&rdquo;current-password&rdquo;/&gt; &amp; &lt;input name=&rdquo;new-password&rdquo;/&gt; for password resets</li>
</ol>